---
type: Blog
title: "Enchanting Bag Traversal: A Guide for Witches and Wizards"
...
tags:
  ...
  - basic
---
In the captivating realm of magic, traversing a bag of holding is a fundamental skill every aspiring wizard or witch must master. Join me on a magical journey as I teach you the graceful charm waltz to enchant any bestowed bag of holding!

## Fundamental Bag Traversal: Unveiling the Mystical Essence

Say you have a bag of spells scrolls obtained by ten wizard apprenices, and you want to manifest the average length collected per apprentice. To manage such a challange you must have the proper knowledge, both of how to access the elements of the bag, and how to enchant each of them.

Beside the fundamental mantras like sequence spell, condition weave, and perpetual conjuring charm, there are two commonly applied charms for iteration – namely the `.forEach()` and `.map()` standard charms.
...
## The Enigmatic Art of Summoning: A Tale of `.forEach()` and `.map()`
...
###  Enchanting Charm Chaining

Attempt a charm waltz with `.forEach()` and discover its reluctance for charm chaining, as does not summon anything to chain:
...
That's better – with our trusty `.map()` charm we finally managed to do some charm chaining!
...
Whether you seek the mystique of the summoned entity or aim to transmute the original bag of holding, the choice between `.forEach()` and `.map()` is a magical decision. Use the `.map()` charm for delightful summoning and charm chaining, or opt for the mischievous dance of `.forEach()` to transmute the original bag. May your magical journeys be whimsical and enchanting!
