    ...
    return (
        <footer className="mt-16 rounded-2xl bg-dark m-10 flex flex-col \
            ...
            <p className="mt-5 px-4 text-center w-3/5 font-light text-base">
                Subscribe to learn about new updates.
            </p>
            <form onSubmit={handleSubmit(onSubmit)} className="mt-6 min-w-[386px] \
                    flex items-stretch bg-light p-2 rounded mx-4">
                <input type="email"
                        placeholder="Enter your email"
                        {...register("email", { required: true, maxLength: 80 })}
                        className="w-full bg-transparent pl-0 text-dark focus:border-dark \
                                focus:ring-0 border-0 border-b mr-2 pb-1" />

                <input type="submit" className="bg-dark text-light cursor-pointer \
